...
// Global value that holds count of reads
								   ...
								   uint16_t offset)
{
	...
	//copy over count to out_buffer 
	// treat uint32_t as a sequence of 4 8-bit memory values to copy over data to uint8_t buffer
	for (int i=0; i<4 ;++i){
		out_buffer[i] = ((uint8_t*)&readCount)[3-i];
	}
